...
\sgcomment{How about the following argument?}
...
Now since $F_{\mathsf{min}}(\mathbb{CH}(\mathbf{C}(S)))$\sgcomment{Check the font here, does not look consistent.} is a closed set, we have that $(c'',d) \in F_{\mathsf{min}}(\mathbb{CH}(\mathbf{C}(S)))$.
