...
While it has been shown in \cite{FGR20} that \textbf{$\mathbf{ASV}$} may not be achievable, in this \mychapter, we show that $\mathbf{ASV}^{\epsilon}$ is indeed achievable. This is expressed in the following theorem.
\begin{theorem}
    \label{ConjAchiev}
...
The rest of this \mychapter is devoted entirely to proving \textbf{\cref{ConjAchiev}}. We start by defining the concept of a witness for $\mathbf{ASV}^{\epsilon}(\sigma_0^i)$.
...
\end{proposition}
% \mbcomment{i vs $k_i$}
We have that $\lim \limits_{i \to \infty} \mathbf{ASV}^{\epsilon}(\sigma_0^i)(v) = c$. We construct a play $\pi_*$ as follows: for $i \in \{1, 2, 3, \dots\}$, the play $\pi_*$ follows the play $\pi_i$ until the mean-payoff of the prefix of the play $\pi_*$ becomes greater than $c_i$. Then the play $\pi_*$ follows the play $\pi_{i+1}$ and so on. We define a strategy $\sigma_0^*$ for Player~0 as follows: if Player~1 follows the play $\pi_*$, then Player~0 follows the play $\pi*$. If Player~1 deviates from $\pi_*$, then Player~0 employs the memoryless strategy described by \cref{RemarkStratEqual}. Clearly, we can see that $\underline{\mathbf{MP}}_1(\pi_*) = c$. We let $\underline{\mathbf{MP}}_1(\pi_*) = d$.
