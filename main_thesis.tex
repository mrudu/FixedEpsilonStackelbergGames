\documentclass{report}
\usepackage{suthesis-2e}

% For combining rows in tables, for colors in tables
\usepackage{multirow}
\usepackage[table,xcdraw]{xcolor}

% For list like - (i), (ii) etc
\usepackage{paralist}
\usepackage{hyperref}

% For Math Symbols
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{comment}

% For Graphs and Automatas
\usepackage{xcolor}
\usepackage{tkz-euclide}
\usetikzlibrary{arrows,automata,positioning}
\usetkzobj{all}

% For cases syntax
\newcounter{casenum}
\newenvironment{caseof}{\setcounter{casenum}{1}}{\vskip.5\baselineskip}
\newcommand{\case}[2]{\vskip.5\baselineskip\par\noindent {\bfseries Case \arabic{casenum}:} #1\\#2\addtocounter{casenum}{1}}

% macros for uniformity
\newcommand{\CS}{\mathbb{C}(S)}
\newcommand{\CH}[1]{{\sf CH}(#1)}
\newcommand{\Fmin}[1]{{\sf F_{\min}}{(#1)}}

% For calling this document as Thesis
\newcommand{\mydoc}{{thesis}}
\newcommand{\mychapter}{{chapter}}
\newcommand{\myChapter}{{Chapter}}

% For Thm, Lem, etc Statements
\usepackage{amsthm}
\usepackage{cleveref}% http://ctan.org/pkg/cleveref
\usepackage{lipsum}% http://ctan.org/pkg/lipsum
\newtheorem{theorem}{Theorem}[chapter]
\newtheorem{corollary}[theorem]{Corollary}
\newtheorem{lemma}[theorem]{Lemma}
\newtheorem{conjecture}[theorem]{Conjecture}
\newtheorem{definition}[theorem]{Definition}
\newtheorem{proposition}[theorem]{Proposition}
\newtheorem{remark}[theorem]{Remark}
\newtheorem{example}[theorem]{Example}

% For referencing within project
\crefname{lemma}{Lemma}{Lemmas}
\crefname{remark}{Remark}{Remarks}
\crefname{theorem}{Theorem}{Theorems}
\crefname{conjecture}{Conjecture}{Conjectures}
\crefname{corollary}{Corollary}{Corollaries}
\crefname{proposition}{Proposition}{Propositions}
\crefname{figure}{Figure}{Figures}
\crefname{example}{Example}{Examples}
\crefname{chapter}{Chapter}{Chapters}
\crefname{section}{Chapter}{Chapters}
\crefname{table}{Table}{Tables}
\Crefname{lemma}{Lemma}{Lemmas}
\Crefname{remark}{Remark}{Remarks}
\Crefname{theorem}{Theorem}{Theorems}
\Crefname{conjecture}{Conjecture}{Conjectures}
\Crefname{corollary}{Corollary}{Corollaries}
\Crefname{proposition}{Proposition}{Propositions}
\Crefname{figure}{Figure}{Figures}
\Crefname{example}{Example}{Examples}
\Crefname{section}{Chapter}{Chapters}
\Crefname{chapter}{Chapter}{Chapters}
\Crefname{table}{Table}{Tables}

% For keeping track of changes
% \newcommand{\sgcomment}[1]{{\color{purple}[#1 - \textbf{Shibashis}]}}
% \newcommand{\mbcomment}[1]{{\color{blue}[#1 - \textbf{Mrudula}]}}
\newcommand{\track}[1]{{\color{red}[#1]}}
\newcommand{\sgcomment}[1]{{}}
\newcommand{\mbcomment}[1]{{}}
% \newcommand{\track}[1]{{}}

% Modifying use of subsections
\renewcommand{\subsection}[1]{{\section*{#1}}}

\begin{document}

\dept{Computer Science}
\title{Stackelberg Mean-payoff Games with One epsilon-Optimal Adversarial Follower}
\author{Mrudula Balachander}
\principaladviser{Shibashis Guha}
\coprincipaladvisor{Jean-Fran\c{c}ois Raskin}
% \firstreader{B Srivatsan}
% \thirdreader{Severus Snape} %if needed
 
\beforepreface
\prefacesection{Abstract}
	\input{abstract}
\prefacesection{Acknowledgments}
	\input{acknowledgments}
\afterpreface

\chapter{Introduction}
	\label{sec:intro}
	\input{Introduction}

\chapter{Preliminaries}
  	\label{sec:prelim}
  	\input{Preliminaries}

\chapter{$\mathbf{ASV}^{\epsilon}$ for a fixed $\epsilon$}
	\label{sec:examples}
	\input{ASVEpsilonExamples}

\chapter{Threshold Problem for $\mathbf{ASV}^{\epsilon}$}
	\label{sec:ThresholdProblem}
	\input{DecisionAlgorithmsASV}
	\section{$\mathbf{ASV}^{\epsilon}_{\mathsf{FM}}$ for a fixed $\epsilon$}
		\label{sec:FMStrategy}
		\input{ExistenceOfASVEpsilonWitnessForFMStrategy}

\chapter{Computing the $\mathbf{ASV}^{\epsilon}$}
	\label{sec:ComputeASV}
	\input{ComputeASV}

\chapter{Existence of Finite Memory Strategies for ASV}
	\label{sec:FMASV}
	\input{FiniteMemForASV}

\chapter{Achievability of $\mathbf{ASV^{\epsilon}}$}
	\label{sec:Achievability}
	\input{Achievability_v2}

\chapter{Conclusions}
\input{conc}

\appendix
	\chapter{A Long Proof}
		\input{ExtensionOfProof-ExpressionsForTau}

\bibliographystyle{plain}
	\bibliography{refs}

\end{document}